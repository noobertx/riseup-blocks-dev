...
function register_block_wprig_productgrid(){
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'render_block_wprig_productgrid'
		)
	);
}
...
function render_block_wprig_productgrid($att)
{
	...
	if ($query->have_posts()) {
		...
		if($enablePagination ){
			$html .= '<div class="wprig-productgrid-pagination">' . wprig_product_pagination_bar($query->max_num_pages, $paged) . '</div>';
		}
		$html .= '</div>';
		wp_reset_postdata();
	}
	...
	add_action('init', 'register_block_wprig_productgrid', 100);
