...
class AccordionBlock extends Component{
    ...
    insertAccordionItem() {
        ...
        insertBlock(createBlock('wprig/accordion-item', newBlockAttributes), undefined, clientId);
    }
    ...
    getAccordionTemplate = (attributes) => {
        ...
        for (let k = 0; k < defaultItems; k++) {
            const content = 'Create stylish call-to-action buttons with wprig Buttons. Play around with typography, design, border and more. Add animations and personalize it to engage visitors instantly.';
            result.push(['wprig/accordion-item', { itemNumber: k, defaultText: content }]);
        }
        return result
    }
    ...
    render (){
        ...
        return(
            <Fragment>
                ...
                <div className={`wprig-block-accordion wprig-block-${uniqueId}${className ? ` ${className}` : ''}`}>
                    <InnerBlocks
                        template={this.getAccordionTemplate(attributes)}
                        allowedBlocks={['wprig/accordion-item']}
                    />
                </div>
