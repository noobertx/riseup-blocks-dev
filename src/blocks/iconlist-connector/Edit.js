...
class Edit extends Component {
    ...
    componentDidMount() {
        ...
        this.placeCaretAtEnd(document.querySelector(`.wprig-block-${uniqueId} .wprig-list-item-text-${this.state.focusedItem}`))
    }
    ...
    renderListItems = () => {
        ...
        return listItems.map((item, index) => {
            return (
                <li className="wprig-list-li wprig-list-li-editor" >
                    <div ref="avoidOnClick" className={`wprig-list-item wprig-list-item-${index}`} onClick={() => this.setState({ currentListItemIndex: index })}>
                        {iconPosition == 'left' && <span className={`wprig-list-item-icon ${item.icon} fa-fw`} onClick={() => this.setState({ openIconPopUp: openIconPopUp ? (currentListItemIndex == index) ? false : true : true })} />}
                        <div
                        ...
                        {iconPosition == 'right' && <span className={`wprig-list-item-icon ${item.icon} fa-fw`} onClick={() => this.setState({ openIconPopUp: openIconPopUp ? (currentListItemIndex == index) ? false : true : true })} />}
                        {
                            item.text.length > 0 &&
                            <Tooltip text={__('Delete this item')}>
                                <span className="wprig-action-remove" role="button"
                                    onClick={() => {
                                        this.updateListItems(index, 'delete')
        ...
        return (
            <Fragment>
                ...
                <div className={`wprig-block-${uniqueId}${className ? ` ${className}` : ''}`}>
                    ...
                    >
                        <ul className="wprig-list wprig-icon-list-connector">
