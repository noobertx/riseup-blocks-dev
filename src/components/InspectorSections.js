...
const Sections = (props) => {
    ...
    const storeName = '_wprig_local_cache_';
    ...
    const _fetchSections = (today) => {
        setLoadingState('wprig-is-loading');
        fetch('http://wpnaked.local/wp-content/uploads/section.json', {
            method: 'GET',
    ...
    const _fetchSection = (section_id, callback) => {
        fetch('https://wprig.io/wp-json/restapi/v2/single-section', {
            method: 'POST',
    ...
    return (
        <PanelBody title={__('Related Sections')} initialOpen={true}>
            <div className='wprig-block-sections'>
                <Tooltip text={__('Sync blocks')}>
                    <button onClick={_syncBlocks} className={'wprig-block-refresh ' + loadingState}>
                        <span className="fas fa-sync-alt"></span>
                    </button>
                </Tooltip>
                {
                    sections.map(section => (
                        <div className='wprig-block-section'>
                            <img width='330' height='230' loading='lazy' src={section.image} alt={section.name} />
                            <div className="wprig-block-section-btns">
