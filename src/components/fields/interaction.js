...
import Timeline from '../interaction-fields/timeline';
import { defaultInteraction } from '../interaction-fields/interactionData';
import { MouseTilt } from '../interaction-fields/interactionParser'
import { _equal } from '../helpers';
...
class Interaction extends Component {
    ...
    componentWillUnmount() {
        const block = $(`.wprig-block-${this.props.uniqueId}`)
        if (block.hasClass('wprig-block-interaction')) {
            block.removeClass('wprig-block-interaction').removeAttr('style')
        }
        if (this.v) {
            this.v.destroy()
            delete this.v
        ...
        if (parentKey === 'mouse_movement') {
            ...
            if (typeof mouse_movement.enable !== 'undefined') {
                ...
                if (mouse_movement.enable && this.state.liveMouseTilt) {
                    setTimeout(() => {
                        let block = document.getElementsByClassName(`wprig-block-${uniqueId}`)
                        if (block.length > 0) {
                            block = block[0]
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (typeof while_scroll_into_view.enable !== 'undefined' && while_scroll_into_view.enable === false) {
            const block = $(`.wprig-block-${uniqueId}`)
            if (block.hasClass('wprig-block-interaction')) {
                block.removeClass('wprig-block-interaction').removeAttr('style')
            }
        }
        ...
        return (
            <Fragment>
                {typeof while_scroll_into_view !== 'undefined' &&
                    <div className="wprig-interaction-view wprig-fields  wprig-field-alt">
                        <div className="sppb-interaction-view-body">
                            {!wprig_admin.pro_enable ?
