...
class Gradient extends Component {
    ...
    defColors = async () => {
        let val = wprig_admin.palette;
        let { colors } = await JSON.parse(localStorage.getItem('wprig-global-settings'));
    ...
    renderColorPicker = (colorType, defaultColor) => {
        const { value } = this.props
        let globalColors = wprig_admin.palette;
        let { colors } = JSON.parse(localStorage.getItem('wprig-global-settings'));
        if (typeof colors !== 'undefined') {
            globalColors = colors
        }
        return (
            <Fragment>
                ...
                <div className="wprig-rgba-palette" style={{ padding: '0px 0px 15px 15px' }}>
                    {globalColors.map((color, index) => <button key={color} style={{ color: color }} onClick={() => this.setSettings(`var(--wprig-color-${index + 1})`, colorType)} />)}
                </div>
            </Fragment>
        )
    }
                        ...
                        gradientColors.map(gradientColor => {
                            ...
                            return (
                                <Dropdown
                                    ...
                                    className={`wprig-gradient-color gradient-color-position-${customClassName}`}
                                    contentClassName="wprig-gradient-color-picker"
                                    renderToggle={({ isOpen, onToggle }) => (
                                        <div className={`wprig-color-picker-container wprig-position-${customClassName}`}>
                                            <Tooltip text={label}>
                                                <button className="wprig-color-picker" isPrimary onClick={onToggle} aria-expanded={isOpen} style={{ backgroundColor: value[color] || defaultColor }} />
                                            </Tooltip>
                                        </div>
                                    )}
                    ...
                    <div className="wprig-field-button-list wprig-ml-auto">
                        {
                            ['linear', 'radial'].map((data, index) => {
                                return (
                                    <button className={(value.type == data ? 'active' : '') + ' wprig-button'} key={index} onClick={() => this.setSettings(data, 'type')}>
                                        {data == 'linear' ?
