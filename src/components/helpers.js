...
export const animationAttr = (data) => {
    if (typeof data !== 'undefined' && typeof data.name !== 'undefined' && data.openAnimation) {
        return { 'data-wpriganimation': JSON.stringify(data) }
    } else {
        return {}
    }
}
    ...
    if (settings.bgType == 'video') {
        if (settings.videoSource == 'local') {
            if (settings.bgVideo && settings.bgVideo.url) {
                return (
                    <div className="wprig-video-bg-wrap">
                        <video className="wprig-video-bg" autoPlay muted loop>
        ...
        if (settings.videoSource == 'external') {
            if (settings.bgExternalVideo) {
                ...
                return (
                    <div className="wprig-video-bg-wrap"><iframe src={src} frameBorder="0" allowFullScreen></iframe></div>
                )
            }
        }
    }
...
export const textAlignment = (alignment) =>{
    var str = "";
    if(alignment){
