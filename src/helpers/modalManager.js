...
class Modal extends Component {
    ...
    render() {
        ...
        return (
            <Fragment>
                <span onClick={e => { this.close() }} className="wprig-pagelist-modal-overlay">&nbsp;</span>
                <div className={`wprig-pagelist-modal-inner`} onClick={e => e.stopPropagation()}   >
                    {this.props.children}
                </div>
            </Fragment>
        );
...
const Manager = {
    open(component) {
        ...
        if (!node) {
            ...
            node.className = "wprig-builder-modal"
            document.body.appendChild(node)
        }
        wp.element.render(component, node)
        document.body.classList.add('wprig-builder-modal-open')
    },
    close() {
        onClose && onClose(() => {
            wp.element.unmountComponentAtNode(node)
            document.body.classList.remove('wprig-builder-modal-open')
        });
    },
}
