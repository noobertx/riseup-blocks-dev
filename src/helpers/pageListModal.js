...
const { apiFetch } = wp;
const { __ } = wp.i18n
import { Modal as wprigModal, ModalManager } from './modalManager'
...
class PageListModal extends Component {
    ...
    componentDidMount() {
        ...
        const options = {
            method: 'POST',
            url: wprig_admin.ajax + '?action=wprig_get_sections',
    ...
    deleteSavedBlock(index, blockID) {
        ...
        const options = {
            method: 'POST',
            url: wprig_admin.ajax + '?action=wprig_delete_saved_block&block_id=' + blockID,
            headers: { 'Content-Type': 'application/json' }
        }
        apiFetch(options).then(response => {
            if (response.success) {
    ...
    importLayoutBlock(itemData, isPro) {
        ...
        if (!wprig_admin.pro_enable && isPro == true) {
            //
        } else {
            ...
            const options = {
                method: 'POST',
                url: wprig_admin.ajax + '?action=wprig_get_single_' + itemType + '&' + itemType + '_id=' + itemData.ID,
                headers: { 'Content-Type': 'application/json' }
            }
            apiFetch(options).then(response => {
                if (response.success) {
                    ...
                    if (importWithGlobal) {
                        ...
                        if (typeof globalSettings.colors !== 'undefined' && globalSettings.colors.length > 0) {
                            globalSettings.colors.forEach((color, index) => {
                                temp = temp.replace(new RegExp(`var.--wprig-color-${index + 1}.`, "g"), color)
                            })
                        }
                        ...
                        if (wprig_admin.import_with_global_settings === 'always') {
                            this.props.insertBlocks(pageData)
                            ModalManager.close();
                        } else if (wprig_admin.import_with_global_settings === 'never') {
                            this.props.insertBlocks(JSON.parse(temp))
                            ModalManager.close();
                        } else if (wprig_admin.import_with_global_settings === 'manually' ||
                            !wprig_admin.import_with_global_settings ||
                            typeof wprig_admin.import_with_global_settings === 'undefined') {
    ...
    _onlickLayoutsTab() {
        ...
        if (!layoutData) {
            ...
            const options = {
                method: 'POST',
                url: wprig_admin.ajax + '?action=wprig_get_layouts',
                headers: { 'Content-Type': 'application/json' }
            }
            apiFetch(options).then(response => {
    ...
    _onlickSavedBlocksTab() {
        ...
        if (!savedBlocks) {
            ...
            const options = {
                method: 'POST',
                url: wprig_admin.ajax + '?action=wprig_get_saved_block',
                headers: { 'Content-Type': 'application/json' }
            }
            apiFetch(options).then(response => {
                this.setState({
    ...
    render() {
        ...
        let types = wprig_admin.pro_enable ? 'active' : 'inactive';
        let {
            itemType,
            blockData,
            layer,
        ...
        const importBlocks = (actionType) => {
            ...
            ModalManager.close();
            if ((wprig_admin.import_with_global_settings === "manually" ||
                !wprig_admin.import_with_global_settings ||
                typeof wprig_admin.import_with_global_settings === 'undefined') && rememberChoice) {
                $.post({
                    url: wprig_urls.ajax,
                    data: {
                        action: 'update_wprig_options',
                        _wpnonce: wprig_urls.nonce,
                        options: {
                            'import_with_global_settings': actionType === 'yes' ? 'always' : 'never',
                        }
                    }
                }).success(function (response) {
                    wprig_admin['import_with_global_settings'] = actionType === 'yes' ? 'always' : 'never'
                }).fail(function (error) {
        ...
        return (
            <Fragment>
                <wprigModal className="wprig-builder-modal-pages-list" customClass="wprig-builder-modal-template-list" onRequestClose={this.props.onRequestClose} openTimeoutMS={0} closeTimeoutMS={0}>

                    <div className="wprig-builder-modal-header">
                        ...
                        <div className="wprig-template-list-header">
                            ...
                            <button className="wprig-builder-close-modal" onClick={e => { ModalManager.close() }} >
                                <i className={"fas fa-times"} />
                            </button>
                        </div>
                    </div>

                    <div className="wprig-layout-modal-sidebar">

                        <div className="wprig-modal-sidebar-content">
                            {
                            ...
                            }
                            {
                                ...
                                <ul className="wprig-template-categories">
                                    <li
                    ...
                    <div className="wprig-layout-modal-content-area">

                        {itemType != 'saved_blocks' && <div className="wprig-template-list-sub-header">
                            <h4>
                                {(this.state.itemType == 'layout' && this.state.layer == 'single') &&
                                    <span className={"wprig-template-back"} onClick={() => this.setState({ layer: 'multiple', parent_id: '' })}><span className="dashicons dashicons-arrow-left-alt" />&nbsp;</span>
                                }
                            ...
                            <div className="wprig-template-filter-button-group">
                                ...
                                <button onClick={() => this._changePriceFilter('pro')} className={'pro' == this.state.priceFilter ? 'active' : ''}>
                                    <img src={wprig_admin.plugin + 'assets/img/icon-premium.svg'} alt="" />
                                    {__('Premium')}
                                </button>
                            </div>
                        </div>}
                        ...
                        {!this.state.loading ?
                            <div id="modalContainer" className="wprig-template-list-modal">
                                <div className="wprig-builder-template-list-container">
                                    {/*<div className="wprig-template-option-header">
                                        ( this.state.itemType == 'layout' ) &&
                                        <div className="template-options">
                                            <ul>
                                                <li className={ this.state.layer === 'multiple' ? 'active' : '' }> <i className="wprig-folder-view" onClick={ e => this.setState( { layer: 'multiple', parent_id: '' } ) } /></li>
                                                <li className={ this.state.layer === 'single' ? 'active' : '' }> <i className="wprig-grid-view" onClick={ e => this.setState( { layer: 'single' } ) } /> </li>
                                            </ul>
                                        </div>

                                </div>*/}

                                    <div id="layouts-blocks-list" className={"wprig-builder-page-templates " + (this.state.itemType == "saved_blocks" ? 'wprig-frontendd-block-list' : '')}>
                                        {
                                            this.state.layer == 'single' && this._sliceCurrentData(currentPageData).map(item => (
                                                <div className="wprig-pagelist-column">
                                                    {
                                                        item.map((data, index) =>
                                                            <SingleItem
                                                                key={index}
                                        ...
                                        {(this.state.layer == 'block' && currentPageData.length != 0) &&
                                            <div className="wprig-reusable-list-title">
                                                <div className="wprig-reusable-list-content">
                                                    <span className="wprig-tmpl-title" > {__('Title')}</span>
                                                </div>
                                                <div className="wprig-reusable-list-info">
                                                    <div className="wprig-reusable-list-info-date">{__('Publish')} </div>
                                                </div>
                                            </div>
                                        }
                                        ...
                                        {(currentPageData.length === 0) &&
                                            <div className="wprig-builder-template-found-empty">
                                                <h3 className="wprig-builder-empty-title"> {this.state.notFoundMessage} </h3>
                                            </div>
                                        }
                            ...
                            <div>
                                <div style={{ height: "600px" }}>
                                    {this.state.requestFailedMsg ?
                                        ...
                                        :
                                        <div className="wprig-modal-loader">
                                            <Spinner />
                                        </div>
                                    }
                                    {/*<button
                                    className="wprig-builder-close-modal"
                ...
                {
                    (isOpen && (wprig_admin.import_with_global_settings === "manually" ||
                        !wprig_admin.import_with_global_settings ||
                        typeof wprig_admin.import_with_global_settings === 'undefined')) && (
                        <Modal
                            title={__('Import Type Settings')}
                            className="wprig-import-global"
                            onRequestClose={closeModal}
                        >
                            <div className="wprig-import-settings">
                                <div className="label">{__("Apply global styling while importing this?")} </div>
                                <div class="wprig-import-settings-footer">
